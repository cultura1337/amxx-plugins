...
#include <hns_xp_skills> // твоё ядро скиллов

#define PLUGIN_NAME    "HNS Skill: PreFog HUD"
#define PLUGIN_VERSION "1.0"
#define PLUGIN_AUTHOR  "cultura"

#define SKILL_MIN_LEVEL 1

// =======================
// Skill
// =======================
new g_iSkillPreFog = -1;

// =======================
// Original PreFog data
// =======================
new g_iHudObject;

new bool:g_bOnOffPre[33];
new bool:g_bOnOffSpeed[33];
...
new PRE_COLOR:g_eSpeedColorDef[33];
new PRE_COLOR:g_eSpeedColorPerf[33];
...
new SPEED_TYPE:g_eSpeedType[33];

new bool:g_isPre[33];

new g_iFog[33];
new bool:g_isOldGround[33];
new bool:g_isOldLadder[33];
new bool:g_isSlide[33];
new bool:g_bInDuck[33];
new Float:g_flOldSpeed[33];
new Float:g_flPreSpeed[33];
new g_iPrevButtons[33];
new bool:g_isSGS[33];

new g_isSpec[33];
new Float:g_flHudTime[33];
new bool:g_isShowPre[33];
new Float:g_flPreShowTime[33];
...
public plugin_init()
{
    ...
    // регистрируем скилл: доступен везде (DM/ZM/PUB)
    g_iSkillPreFog = hns_skills_register(
        "PreFog HUD",
        SKILL_MIN_LEVEL,
        SKILL_MODE_DM | SKILL_MODE_ZM | SKILL_MODE_PUB
    );
...
public rgPM_Move(id)
{
    // если скилл не зарегистрирован — ничего
    if (g_iSkillPreFog < 0)
        return HC_CONTINUE;

    // ВАЖНО: если скилл выключен — вообще ничего не считаем
    if (!hns_skills_is_enabled(id, g_iSkillPreFog))
        return HC_CONTINUE;

    // если у игрока всё выключено — тоже не считаем
    ...
    else
    {
        ...
        if (g_isOldGround[id])
        {
            ...
            else
            {
                if (g_iFog[id] > 10)
                {
                    if (isDuck) format_prest(id, PRE_DUCK, g_flOldSpeed[id]);
                    if (isJump) format_prest(id, PRE_JUMP, g_flOldSpeed[id]);
                }
                else
                {
...
public cmdPreSpeedMenu(id)
{
    ...
    if (g_iSkillPreFog < 0 || !hns_skills_is_enabled(id, g_iSkillPreFog))
    {
        client_print_color(id, print_team_blue, "[^3PreFog^1] Этот HUD доступен через /skills (PreFog HUD).");
        return PLUGIN_HANDLED;
    }

    new hMenu = menu_create("\rPreFog menu:", "SpeedMenuCode");

    menu_additem(hMenu, g_bOnOffSpeed[id] ? "Speed - \yon" : "Speed - \doff", "1");
    menu_additem(hMenu, g_bOnOffPre[id]   ? "Prefog - \yon" : "Prefog - \doff", "2");
...
public SpeedMenuCode(id, hMenu, item)
{
    ...
    switch (item)
    {
        case 0: { cmdShowSpeed(id); cmdPreSpeedMenu(id); }
        case 1: { cmdShowPre(id);   cmdPreSpeedMenu(id); }
        case 2:
        {
            switch (g_eSpeedType[id]) {
...
public cmdShowPre(id)
{
    ...
    g_bOnOffPre[id] = !g_bOnOffPre[id];
    client_print_color(id, print_team_blue, "[^3PreFog^1] Show Fog/Prestrafe: %s", g_bOnOffPre[id] ? "^3ON^1" : "^3OFF^1");
    return PLUGIN_HANDLED;
}
...
public cmdShowSpeed(id)
{
    ...
    g_bOnOffSpeed[id] = !g_bOnOffSpeed[id];
    client_print_color(id, print_team_blue, "[^3PreFog^1] Show Speed: %s", g_bOnOffSpeed[id] ? "^3ON^1" : "^3OFF^1");
    return PLUGIN_HANDLED;
}
...
stock show_prespeed(id, Float:flSpeed, Float:flSpeedDef = 0.0)
{
    ...
    // показываем ТОЛЬКО владельцу (самому игроку со скиллом)
    set_hudmessage(iColors[0], iColors[1], iColors[2], g_pCvar[c_iPreHudX], g_pCvar[c_iPreHudY], 0, 1.0, 0.15, 0.0, 0.0, g_pCvar[c_iPreHud]);

    if (g_bOnOffPre[id] && g_isShowPre[id] && (g_eHudPre[id][HUD_POST] > 30.0))
    {
        if (g_eHudPre[id][HUD_TYPE] == PRE_FOG)
        {
            ShowSyncHudMsg(id, g_iHudObject, "%s^n^n%d %s^n%.2f^n%.2f",
                szSpeed,
                g_eHudPre[id][HUD_FOG],
                g_szFogType[g_eHudPre[id][HUD_FOGTYPE]],
                g_eHudPre[id][HUD_PREST],
                g_eHudPre[id][HUD_POST]
            );
        }
        else
        {
            ShowSyncHudMsg(id, g_iHudObject, "%s^n^n%s^n%.2f",
                szSpeed,
                g_szPreType[g_eHudPre[id][HUD_TYPE]],
                g_eHudPre[id][HUD_POST]
            );
        }
    }
    ...
    {
        ShowSyncHudMsg(id, g_iHudObject, "%s", szSpeed);
    }
...
FormatRGBHud(id, const Float:val, colors[3])
{
    ...
    switch (g_eSpeedColorPerf[id])
    {
        case CLR_WHITE:  iColorPerf = {255, 255, 255};
        case CLR_GREEN:  iColorPerf = {0, 250, 0};
        case CLR_VIOLET: iColorPerf = {250, 0, 250};
        case CLR_BLUE:   iColorPerf = {0, 150, 250};
        case CLR_RED:    iColorPerf = {250, 0, 0};
        case CLR_YELLOW: iColorPerf = {250, 250, 0};
    }

    switch (g_eSpeedColorDef[id])
    {
        case CLR_WHITE:  iColorDef = (g_eHudPre[id][HUD_FOGTYPE] == FOG_PERFECT && g_eHudPre[id][HUD_PREST] > 30.0) ? iColorPerf : {255, 255, 255};
        case CLR_GREEN:  iColorDef = (g_eHudPre[id][HUD_FOGTYPE] == FOG_PERFECT && g_eHudPre[id][HUD_PREST] > 30.0) ? iColorPerf : {0, 250, 0};
        case CLR_VIOLET: iColorDef = (g_eHudPre[id][HUD_FOGTYPE] == FOG_PERFECT && g_eHudPre[id][HUD_PREST] > 30.0) ? iColorPerf : {250, 0, 250};
        case CLR_BLUE:   iColorDef = (g_eHudPre[id][HUD_FOGTYPE] == FOG_PERFECT && g_eHudPre[id][HUD_PREST] > 30.0) ? iColorPerf : {0, 150, 250};
        case CLR_RED:    iColorDef = (g_eHudPre[id][HUD_FOGTYPE] == FOG_PERFECT && g_eHudPre[id][HUD_PREST] > 30.0) ? iColorPerf : {250, 0, 0};
        case CLR_YELLOW: iColorDef = (g_eHudPre[id][HUD_FOGTYPE] == FOG_PERFECT && g_eHudPre[id][HUD_PREST] > 30.0) ? iColorPerf : {250, 250, 0};
    }
...
{
    new Float:flMaxSpeed = get_entvar(id, var_maxspeed);
